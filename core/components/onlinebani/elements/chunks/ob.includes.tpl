<!-- Include all compiled plugins (below), or include individual files as needed -->
<script src="[[++assets_url]]components/bootstrap/dist/js/bootstrap.min.js"></script>

<link href="{$_modx->config['onlinebani_assets_url']}css/web/ob_all_css.css" rel="stylesheet" media="screen">
<link href="{$_modx->config['onlinebani_assets_url']}css/web/obowner.css" rel="stylesheet" media="screen">

<link href="//cdn.datatables.net/1.10.16/css/jquery.dataTables.min.css" rel="stylesheet" media="screen">
<script type="text/javascript" src="//cdn.datatables.net/1.10.16/js/jquery.dataTables.min.js"></script>
<script type="text/javascript" src="{$_modx->config['onlinebani_assets_url']}js/web/obmodel.js"></script>
<script type="text/javascript" src="{$_modx->config['onlinebani_assets_url']}js/web/ob.js"></script>

<script type="text/javascript" src="assets/components/ms2guploader/vendor/plupload/js/plupload.full.min.js"></script>



